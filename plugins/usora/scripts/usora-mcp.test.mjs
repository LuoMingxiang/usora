...
test("activity_capture writes and merges in the chosen Hub directory", async t => {
  const cwd = await mkdtemp(path.join(os.tmpdir(), "usora-mcp-"));
  t.after(() => rm(cwd, { recursive: true, force: true }));
  const dataDir = path.join(cwd, "hub");
  const requests = [
  ...
  assert.equal(responses[3].result.content[0].type, "text");
  assert.match(responses[4].error.message, /letters, numbers, and hyphens/);
  const [file] = await readdir(path.join(dataDir, "activities"));
  const activity = JSON.parse(await readFile(path.join(dataDir, "activities", file), "utf8"));
...
test("data mutations are refused until a directory is chosen", async t => {
  const cwd = await mkdtemp(path.join(os.tmpdir(), "usora-mcp-"));
  t.after(() => rm(cwd, { recursive: true, force: true }));

  const requests = [
    { jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "test", version: "1" } } },
    { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "activity_capture", arguments: { session_id: "s1", task: "t", result: "r" } } },
  ...
  const responses = output.trim().split("\n").map(JSON.parse);
  // activity_capture must be refused with a clear "not initialized" error.
  assert.match(responses[1].error.message, /not initialized/);

  // hub_status must report the unlocated state.
  const status = JSON.parse(responses[2].result.content[0].text);
  assert.equal(status.located, false);
