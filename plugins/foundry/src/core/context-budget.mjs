...
import { dirPath, now, readJson, writeEvent } from "./storage.mjs";
...
function partReport(stage, parts = {}) {
  ...
  const entries = Object.fromEntries(
    ["required", "recommended", "optional"].map((kind) => {
...
export async function checkContextBudget(stage, parts = {}) {
  const report = partReport(stage, parts);
  if (report.overflow) await writeEvent("ContextBudgetOverflow", report);
  return report;
}

export async function recordIntelligenceRun(args = {}) {
...
export async function handleContextBudget(args = {}) {
...
export async function handleTelemetryMetrics() {
  ...
  const patternIndex = await readJson(path.join(await dirPath("indexes"), "patterns.json"), {
    patterns: [],
  });
  ...
  return {
    ...
    avg_context_chars: average(runs.map((run) => run.input_chars || 0)),
    avg_evidence_loaded: average(runs.map((run) => run.evidence_loaded || 0)),
