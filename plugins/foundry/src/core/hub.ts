...
type HubConfigArgs = HubInitArgs & {
  path?: string;
};
type HubCleanupArgs = {
  mode?: string;
...
export async function handleHubInit(args: HubInitArgs = {}) {
 ...
 */
export async function handleHubConfig(args: HubConfigArgs = {}) {
  const config = await loadConfig();
 ...
 */
export async function handleHubCleanup(args: HubCleanupArgs = {}) {
...
async function archiveGenerated() {
  ...
  for (const file of await fs.readdir(activitiesDir)) {
    ...
    if (isRecord(item) && (ARCHIVABLE_STATES.includes(String(item.state)) || item.skill_id)) {
