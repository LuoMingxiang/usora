...
async function main() {
  ...
  const candidate = JSON.parse(text(responses.at(-3))).candidate;
  ...
  const events = JSON.parse(text(moreResponses.at(-2))).events;
  const telemetry = JSON.parse(text(moreResponses.at(-1)));
