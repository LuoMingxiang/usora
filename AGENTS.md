...
Use `hub_init`, `hub_status`, `hub_cleanup`, `candidate_create`, `candidate_evaluate`, and `skill_publish` through MCP. For cleanup, use `generated` by default; use `all` only when the user explicitly requests all Activity deletion and passes confirmation. Never ask the user to install Python or run a CLI.
