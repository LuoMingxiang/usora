...
const mcpDist = path.join(foundryRoot, "dist");
const sessionHook = path.join(foundryRoot, "dist", "session-hook.js");

async function installBuiltRuntime(pluginRoot) {
  await mkdir(path.join(pluginRoot, "dist"), { recursive: true });
  await cp(mcpDist, path.join(pluginRoot, "dist"), { recursive: true });
...
test("hub_init detects CodeBuddy marketplace installs without env vars", async (t) => {
  ...
  await installBuiltRuntime(marketplaceRoot);
  t.onTestFinished(() => rm(cwd, { recursive: true, force: true }));
  t.onTestFinished(() => rm(home, { recursive: true, force: true }));

  const child = spawn(process.execPath, [path.join(marketplaceRoot, "dist", "mcp.js")], {
    cwd,
...
test("hub_init detects Codex cache installs without env vars", async (t) => {
  ...
  await installBuiltRuntime(pluginRoot);
  t.onTestFinished(() => rm(cwd, { recursive: true, force: true }));
  t.onTestFinished(() => rm(home, { recursive: true, force: true }));

  const child = spawn(process.execPath, [path.join(pluginRoot, "dist", "mcp.js")], {
    cwd,
    env: { ...process.env, HOME: home, USERPROFILE: home },
    stdio: ["pipe", "pipe", "inherit"],
  });
...
test("plugin_cache_cleanup is exposed and safe outside installed cache", async (t) => {
  ...
  for (const name of ["hub_init", "activity_capture", "candidate_create", "skill_create", "governance_scan"]) {
...
test("tools/call rejects invalid arguments predictably", async (t) => {
  const cwd = await mkdtemp(path.join(os.tmpdir(), "usora-mcp-"));
  t.onTestFinished(() => rm(cwd, { recursive: true, force: true }));
  ...
  assert.match(responses[1].error.message, /arguments must be an object/);
});
